...
//         completed: false,
//         file: selectedFile ? selectedFile.base64 : '',
//         fileType: selectedFile ? selectedFile.type : '',
//       };
//       try {
//         const response = await axios.post(`http://localhost:3000/tasks`, { listId, task: taskData });
...
//       console.log('File selected:', result); // Debugging log
//       const base64 = await convertFileToBase64(result.uri);
//       setSelectedFile({ uri: result.uri, name: result.name, type: result.mimeType, base64 });
...
//     if (!result.canceled) {
//       console.log('Image selected:', result); // Debugging log
//       setSelectedFile({ uri: result.uri, name: result.uri.split('/').pop(), type: result.type, base64: result.base64 });
//       setIsFilePickerVisible(false);
//     }
//   };
...
//     setSelectedFile(task.file ? { uri: task.file, base64: task.file, type: task.fileType } : null);
...
//           file: selectedFile ? selectedFile.base64 : currentTask.file,
//           fileType: selectedFile ? selectedFile.type : currentTask.fileType,
//         };
...
//                         <Text style={styles.fileName}>{item.file.split('/').pop()}</Text>
//                       </View>
//                     )}
//                   </View>
...
//                     console.log('Attach File button pressed'); // Debugging log
...
//         >
//           <TouchableWithoutFeedback onPress={() => setIsFilePickerVisible(false)}>
//             <View style={styles.modalContainer}>
//               <TouchableWithoutFeedback>
//                 <View style={styles.modalContent}>
...
//                     <Text style={styles.dateButton}>Files</Text>
//                   </TouchableOpacity>
//                   <TouchableOpacity style={styles.saveButton} onPress={() => setIsFilePickerVisible(false)}>
    ...
    <GestureHandlerRootView style={{ flex: 1 }}>
      <View style={styles.container}>
        ...
        <FlatList
          ...
          renderItem={({ item }) => (
            <Swipeable renderRightActions={() => renderRightActions(item._id)}>
              <TouchableOpacity onPress={() => navigation.navigate('TaskDetails', { task: item, listId, fetchTasks })}>
                <View style={styles.taskContainer}>
                  <TouchableOpacity onPress={() => toggleCompleteTask(item._id)}>
                    <MaterialIcons name={item.completed ? "check-circle" : "radio-button-unchecked"} size={24} color={item.completed ? "#22A39F" : "#222831"} />
                  </TouchableOpacity>
                  <View style={{ flex: 1 }}>
                    ...
                    {item.files && item.files.length > 0 && (
                      <View style={styles.fileInfoContainer}>
                        <MaterialIcons name="attach-file" size={20} color="#222831" />
                        <Text style={styles.fileInfo}>{item.files.length}</Text>
                      </View>
                    )}
                  </View>
                  <View style={styles.dueDateContainer}>
        ...
        <TouchableOpacity style={styles.addButton} onPress={() => navigation.navigate('TaskDetails', { task: null, listId, fetchTasks })}>
          <AntDesign name="pluscircle" size={50} color="#22A39F" />
        </TouchableOpacity>
      </View>
    </GestureHandlerRootView>
