...
const addToCalendar = async () => {
  try {
    const response = await axios.post('http://localhost:3000/create-event', {
      ...
      endDateTime: new Date(selectedDate.getTime() + 60 * 60 * 1000).toISOString(), // 1 hour later
    });
    console.log('Event URL:', response.data.eventUrl);
    Alert.alert('Success', 'Event added to Google Calendar');
  } catch (error) {
    console.error('Error adding event to Google Calendar:', error);
    Alert.alert('Error', 'Could not add event to Google Calendar. Please try again later.');
  }
};
  ...
  return (
    <ScrollView style={styles.container}>
      <View style={styles.paddingContainer}>
        ...
        <TouchableOpacity style={styles.optionContainer} onPress={addToCalendar}>
