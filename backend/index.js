...
app.use(bodyParser.json({ limit: '50mb' })); // Increase limit to handle large base64 files
app.use(cors());
...
const TaskSchema = new mongoose.Schema({
  ...
  completed: Boolean,
  file: String,
  fileType: String, // Store file type to help with display
});
...
app.put('/tasks/:listId/:taskId', async (req, res) => {
  const { listId, taskId } = req.params;
  console.log('Updating task with ID:', taskId, 'in list with ID:', listId); // Debugging log
  const { name, description, dueDate, completed, file, fileType } = req.body;
  const list = await List.findById(listId);
  if (list) {
    const task = list.tasks.id(taskId);
    if (task) {
